...
def wgs84_2ENU(coords,NW_coords=None,SE_coords=None):
    ...
    validateCoords(coords,NW_coords,SE_coords)

    if coords.shape[1] == 2:
        zeroes = np.zeros((coords.shape[0],1))
        coords = np.hstack((coords,zeroes))
        print(f"Assuming Z = 0 for all points")

    assert coords.shape[1] == 3, "Number of columns should be 3"

    
    NW_lat, NW_lon = NW_coords
    SE_lat, SE_lon = SE_coords
    C_lat, C_lon = (NW_lat + SE_lat) / 2, (NW_lon + SE_lon) / 2
    ...
    C_X, C_Y, C_Z = geodetic2ECEF(C_lat, C_lon, 0)
